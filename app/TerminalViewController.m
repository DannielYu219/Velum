...
// hasExternalKeyboard is now declared in the public header (TerminalViewController.h)
// so the SwiftUI wrapper can force-set it to suppress the extra-keys bar flash.
...
- (void)_updateStyleFromPreferences:(BOOL)animated {
    ...
    // Velum: 彻底移除 extra keys bar，不再条件判断
...
- (NSArray<UIKeyCommand *> *)keyCommands {
    // Velum Phase 2: disable iSH's built-in key commands.
    // All keyboard shortcuts are now routed through SwiftUI's VelumControl
    // (Control Plane) so they don't collide with the desktop shell.
    return nil;
}
